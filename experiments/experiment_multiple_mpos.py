...
def generate_symmetrical_mpos_symmetricly(d, D, N, K, is_mpop=False):
    ...
    for _ in range(K):
        ...
        A = [rng.standard_normal((d, D, D), dtype=np.float32) for _ in range(N)]
...
def calc_trace_of_product_mpo(mpos_input):
    """
    Calculates the trace of the product of multiple MPOs: Tr(MPO_0 * MPO_1 * ... * MPO_K-1).

    Args:
        mpos_input: List of MPOs. Each MPO is a list of Nodes.

    Returns:
        The scalar value of the trace.
    """
    ...
    for j in range(N):
        ...
        # C. Contract the Column
        #    Contract down the column to merge all K nodes into one
        C = col[0]
        for k in range(1, K):
            C = tn.contract_between(C, col[k])

        # D. Contract the Trace Edge (now a self-loop on C)
        C = tn.contract_trace_edges(C)

        site_results.append(C)

    # 4. Contract the Horizontal Chain
    final_node = site_results[0]
    for j in range(1, N):
        final_node = tn.contract_between(final_node, site_results[j])

    # 5. Return the scalar value
    return final_node.tensor.item()
...
def create_embedding_tree(m, d, D, name):
    ...
    v = np.einsum("abc, eda -> cbde", v_1, v_2).reshape(m,m,m).astype(np.float32)
    v = Node(v, name=name)
    return v
...
def create_sketching_network_tn(N, d, m, D):
    ...
    spine_node = create_embedding_tree(m, d,D, "Core_0")
    all_nodes.append(spine_node)

    # Actual Connections using tn.connect
    tn.connect(leaves[0][1], spine_node[0])  # Connect Leaf 0 to Core Input 0
    tn.connect(leaves[1][1], spine_node[1])  # Connect Leaf 1 to Core Input 1

    # The output of this fusion
    current_bond_edge = spine_node[2]

    # Iterative Steps: Fuse the rest of the leaves (Leaf 2 to Leaf N-1)
    for i in range(2, N):
        # Create next core tensor
        new_spine_node = create_embedding_tree(m, d,D, name=f"Core_{i - 1}")
        all_nodes.append(new_spine_node)

        # Connect the accumulated spine (current_bond_edge) to the new core
        tn.connect(current_bond_edge, new_spine_node[0])

        # Connect the next leaf (Leaf i) to the new core
        tn.connect(leaves[i][1], new_spine_node[1])

        # Update the bond edge to be the output of this new core
        current_bond_edge = new_spine_node[2]
...
def sketch_k_mpos(N, d, m, K, D, mpos):
    """
...
if __name__ == "__main__":
    ...
    for i in range(1,M+1):
        ...
        expectation += sketch_k_mpos(N, d, m, K, D, curr_mpos)
    ...
    plt.plot(np.arange(1,M+1)[100::10], errors[100::10])
    plt.show(block=True)
